...
PerceptionUtils::PerceptionUtils(ros::NodeHandle& nh) {
  ...
  nh.param("perception_utils/top_angle_pred", top_angle_pred, -1.0);
  nh.param("perception_utils/left_angle_pred", left_angle_pred, -1.0);
  nh.param("perception_utils/right_angle_pred", right_angle_pred, -1.0);
  ...
  n_top_p << 0.0, sin(M_PI_2 - top_angle_pred), cos(M_PI_2 - top_angle_pred);
  n_bottom_p << 0.0, -sin(M_PI_2 - 2*top_angle_pred), cos(M_PI_2 - 2*top_angle_pred);

  n_left_p << sin(M_PI_2 - left_angle_pred), 0.0, cos(M_PI_2 - left_angle_pred);
  n_right_p << -sin(M_PI_2 - right_angle_pred), 0.0, cos(M_PI_2 - right_angle_pred);
...
void PerceptionUtils::setPose(const Vector3d& pos, const double& yaw) {
  ...
  normals_p = { n_top_p, n_bottom_p, n_left_p, n_right_p };
  for (auto& np : normals_p)
    np = R_wc * np;
}
...
bool PerceptionUtils::insideCtrl(const Vector3d& point)
{
  Eigen::Vector3d dir = point - pos_;
  if (dir.norm() > max_dist_) return false;

  dir.normalize();
  for (auto n : normals_p) {
    if (dir.dot(n) < 0.0) return false;
  }
  return true;
}

...
void Astar::init(ros::NodeHandle& nh, const EDTEnvironment::Ptr& env) {
  ...
  nh.param("astar/lambda_heu_global", lambda_heu_global_, -1.0);
...
int Astar::global_search(const Eigen::Vector3d& start_pt, const Eigen::Vector3d& end_pt)
{
  NodePtr cur_node = path_node_pool_[0];
  cur_node->parent = NULL;
  cur_node->position = start_pt;
  posToIndex(start_pt, cur_node->index);
  cur_node->g_score = 0.0;
  cur_node->f_score = lambda_heu_global_ * getDiagHeu(cur_node->position, end_pt);
    ...
    for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) {
          ...
          // Check safety
          if (!edt_env_->sdf_map_->isInBox(nbr_pos)) continue;
          if (edt_env_->sdf_map_->get_PredStates(nbr_pos) == SDFMap::PRED_INTERNAL)
            continue;
          ...
          for (double l = 0.1; l < len; l += 0.1) {
            Vector3d ckpt = cur_pos + l * dir;
            if (edt_env_->sdf_map_->get_PredStates(ckpt) == SDFMap::PRED_INTERNAL) {
              safe = false;
              break;
            }
          }
          ...
          neighbor->parent = cur_node;
          neighbor->g_score = tmp_g_score;
          neighbor->f_score = tmp_g_score + lambda_heu_global_ * getDiagHeu(nbr_pos, end_pt);
          open_set_.push(neighbor);
          open_set_map_[nbr_idx] = neighbor;
        }
  }
...
int Astar::local_search(const Eigen::Vector3d& start_pt, const Eigen::Vector3d& end_pt)
{
  NodePtr cur_node = path_node_pool_[0];
  cur_node->parent = NULL;
  cur_node->position = start_pt;
  ...
  while (!open_set_.empty()) {
    ...
    if ((ros::Time::now() - t1).toSec() > max_search_time_) {
      // std::cout << "early";
      std::cout << "search time:" << (ros::Time::now() - t1).toSec() << std::endl;
      early_terminate_cost_ = cur_node->g_score + getDiagHeu(cur_node->position, end_pt);
      ROS_WARN("Astar local search over time!");
      return NO_PATH;
    }
    ...
    for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) {
          ...
          // Check safety
          if (!edt_env_->sdf_map_->isInBox(nbr_pos)) continue;
          if (getInternal(nbr_pos, 10) == 1 ||
          getInfOcc(nbr_pos, 3) == 1)
            continue;
...
int Astar::getInternal(const Eigen::Vector3d& pos, const int& step)
{
  int flag = 0;
  ...
  for (int x = -step; x <= step; ++x)
    for (int y = -step; y <= step; ++y)
      for (int z = -5; z <= 5; ++z) {
        inflate_idx(0) = id(0) + 5*x;
        inflate_idx(1) = id(1) + 5*y;
        inflate_idx(2) = id(2) + z;
        if (edt_env_->sdf_map_->isInMap(inflate_idx))
        {
          inflate_address = edt_env_->sdf_map_->toAddress(inflate_idx);
          if (edt_env_->sdf_map_->get_PredStates(inflate_idx) == SDFMap::OCCUPANCY::PRED_INTERNAL)
          {
            flag = 1;
            break;
          }
...
int Astar::getInfOcc(const Eigen::Vector3d& pos, const int& step)
{
  int flag = 0;
  Eigen::Vector3i inflate_idx, id;
  edt_env_->sdf_map_->posToIndex(pos, id);
  ...
  for (int x = -step; x <= step; ++x)
    for (int y = -step; y <= step; ++y)
      for (int z = -step; z <= step; ++z) {
        ...
        {
          inflate_address = edt_env_->sdf_map_->toAddress(inflate_idx);
          if (edt_env_->sdf_map_->getInflateOccupancy(inflate_idx) == 1)
          {
            flag = 1;
            break;
          }

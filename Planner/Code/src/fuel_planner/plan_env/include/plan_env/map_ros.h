...
private:
  ...
  void normal_vis();
  int normal_judge(Eigen::Vector3d& center, Eigen::Vector3d& pos, float& coeff);
  void sample_vp_global(cluster_normal& set, int& qualified_vp);
  ...
  void fullCostMatrix(const Eigen::Vector3d& cur_pos, const Eigen::Vector3d& cur_vel, const Eigen::Vector3d cur_yaw,
    Eigen::MatrixXd& mat);
  void global_path(const Eigen::Vector3d& cur_pos, const Eigen::Vector3d& cur_vel, const Eigen::Vector3d cur_yaw,
    vector<int>& order);

...
public:
  ...
  void ViewDijkstraSearch(const int& start, const int& goal, vector<shared_ptr<NodeT>>& path);
...
template <typename NodeT>
void GraphSearch<NodeT>::ViewDijkstraSearch(const int& start, const int& goal,
                                        vector<shared_ptr<NodeT>>& path)
{
  priority_queue<shared_ptr<NodeT>, vector<shared_ptr<NodeT>>, NodeCompare<NodeT>> open_set;
  std::cout << "Node: " << node_num_ << ", edge: " << edge_num_ << std::endl;
  ...
  while (!open_set.empty()) {
    ...
    for (auto vb : vc->neighbors_) {
      ...
      // Add new node or updated node in open set
      double g_tmp = vc->g_value_ + vc->localCostTo(vb);
      if (g_tmp < vb->g_value_) {
        vb->g_value_ = g_tmp;
        vb->parent_ = vc;
        open_set.push(vb);

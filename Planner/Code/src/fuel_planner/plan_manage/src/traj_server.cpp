...
ros::Publisher cmd_vis_pub, pos_cmd_pub, traj_pub, mesh_pub;
nav_msgs::Odometry odom;
quadrotor_msgs::PositionCommand cmd;
tf::TransformBroadcaster* broadcaster;

// Info of generated traj
vector<NonUniformBspline> traj_, last_traj_;
double traj_duration_, last_traj_duration_;
ros::Time start_time_, last_start_time_;
int traj_id_, last_traj_id_, exec_id_;
int pub_traj_id_, last_pub_traj_id_;
...
void drawFOV(const vector<Eigen::Vector3d>& list1, const vector<Eigen::Vector3d>& list2, const double& yaw) {
  visualization_msgs::Marker mk;
  mk.header.frame_id = "world";
  mk.header.stamp = ros::Time::now();
  mk.id = 0;
  ...
  meshROS.scale.y = 2.5;
  meshROS.scale.z = 3.0;
  meshROS.mesh_resource = "file:///home/albert/UAV_Planning/fuel_airsim/src/fuel_planner/utils/hummingbird.mesh";
  ...
  transform.setOrigin(tf::Vector3(list1[0](0), list1[0](1), list1[0](2)));
  transform.setRotation(tf::Quaternion(0.0, 0.0, sin(0.5*yaw_mesh), cos(0.5*yaw_mesh)));

  broadcaster->sendTransform(tf::StampedTransform(transform,   meshROS.header.stamp, string("world"),  string("base"))); 
...
void bsplineCallback(const bspline::BsplineConstPtr& msg) {
  ...
  double dt = (ros::Time::now() - msg->start_time).toSec();
  ROS_ERROR("receive time interval.");
  cout << dt << "s" << endl;
  if (dt < 0 || msg->traj_id == 1)
  {
    ...
    last_traj_id_ = traj_id_;
    start_time_ = msg->start_time;
    traj_id_ = msg->traj_id;
    last_traj_ = traj_;
    last_traj_duration_ =  traj_duration_;
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  double t_last = (time_now - last_start_time_).toSec();
  ...
  if ((t_cur < traj_duration_ && t_cur >= 0.0) || traj_id_ == 1) {
  ...
  else if (t_cur < 0.0 && t_last < last_traj_duration_)
  {
    pos = last_traj_[0].evaluateDeBoorT(t_last);
    vel = last_traj_[1].evaluateDeBoorT(t_last);
    acc = last_traj_[2].evaluateDeBoorT(t_last);
    yaw = last_traj_[3].evaluateDeBoorT(t_last)[0];
    yawdot = last_traj_[4].evaluateDeBoorT(t_last)[0];
    jer = last_traj_[5].evaluateDeBoorT(t_last);
    exec_id_ = last_traj_id_;
...
{
  double x1 = max(fabs(a), fabs(b));  
  double x2 = max(fabs(x1), fabs(c)); 

  return x2;
}

void collect_traj(Eigen::MatrixXd &path)
{
  int pt_num = path.rows();
  ...
  for (int i=0; i<pt_num; ++i)
  {
    ...
    if(i>0)
    {
      x_step = path(i,0)-path(i-1,0);
      y_step = path(i,1)-path(i-1,1);
      z_step = path(i,2)-path(i-1,2);
    }
    else
    {
      x_step = path(i,0)-0.0;
    ...
    double step = max_three(x_step, y_step, z_step);

    int step_int = (int)(step/0.01);
    int yaw_step_int = abs((int)(path(i,3)/0.005));
    for (int j=0; j<step_int; ++j)
    {
      cmd.position.x += (x_step/step_int);
      cmd.position.y += (y_step/step_int);
      cmd.position.z += (z_step/step_int);
      cmd.yaw += (path(i,3)/step_int);
      pos_cmd_pub.publish(cmd);
...
int main(int argc, char** argv) {
  ...
  mesh_pub = node.advertise<visualization_msgs::Marker>("/odom_visualization/robot", 10);
  ...
  nh.param("reconfsm/replan_time", replan_time_, 0.1);

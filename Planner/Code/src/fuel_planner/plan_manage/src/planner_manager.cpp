...
void FastPlannerManager::initPlanModules(ros::NodeHandle& nh) {
  ...
  nh.param("exploration/vm", vmax_, -1.0);
...
void FastPlannerManager::planReconTraj(const vector<Eigen::Vector3d>& tour, const Eigen::Vector3d& cur_vel,
                       const Eigen::Vector3d& cur_acc, const double& time_lb)
{
  if (tour.empty()) ROS_ERROR("Empty path to traj planner");
  vector<Eigen::Vector3d> tmp_tour;

  double dura = 0.0;
  Eigen::Vector3d pos = tour[0];
  Eigen::Vector3d vel = cur_vel;
  Eigen::Vector3d acc = cur_acc;
  vector<NonUniformBspline> all_pos_trajs;
  double duration_ = 0.0;
  NonUniformBspline pos_traj, vel_traj, acc_traj;
  int pt_size = 0, knot_size = 0, seg_size = 0;
  vector<Eigen::MatrixXd> ctrl_pts_all;

  for (int i=0; i<tour.size()-1; ++i)
  {
    tmp_tour = {pos, 0.5*(pos+tour[i+1]), tour[i+1]};
    double time_tmp_lb = (tour[i+1]-pos).norm() / vmax_;
    const int pt_num = tmp_tour.size();
    Eigen::MatrixXd pos(pt_num, 3);
    for (int i = 0; i < pt_num; ++i) pos.row(i) = tmp_tour[i];
    ...
    PolynomialTraj init_traj;
    PolynomialTraj::waypointsTraj(pos, vel, zero, acc, zero, times, init_traj);
    // B-spline-based optimization
    vector<Vector3d> points, boundary_deri;
    double duration = init_traj.getTotalTime();
    int seg_num = init_traj.getLength() / pp_.ctrl_pt_dist;
    ...
    tmp_traj.getBoundaryStates(2, 0, start, end);
    bspline_optimizers_[0]->setBoundaryStates(start, end);
    if (time_tmp_lb > 0) bspline_optimizers_[0]->setTimeLowerBound(time_tmp_lb);

    bspline_optimizers_[0]->optimize(ctrl_pts, dt, cost_func, 1, 1);
    pos_traj.setUniformBspline(ctrl_pts, pp_.bspline_degree_, dt);
    ctrl_pts_all.push_back(ctrl_pts);
    vel_traj = pos_traj.getDerivative();
    acc_traj = vel_traj.getDerivative();

    dura = pos_traj.getTimeSum();
    duration_ += dura;
    pt_size += pos_traj.getControlPoint().rows();
    knot_size += pos_traj.getKnot().rows();
    pos = pos_traj.evaluateDeBoor(dura);
    vel = vel_traj.evaluateDeBoor(dura);
    acc = acc_traj.evaluateDeBoor(dura);
    all_pos_trajs.push_back(pos_traj);
  }
  
  Eigen::MatrixXd pos_pts_all(pt_size, 3);
  Eigen::VectorXd knots_all(knot_size);
  int counter_p = 0;
  int counter_k = 0;
  // for (int i=0; i<all_pos_trajs.size(); ++i)
  // {
  //   Eigen::MatrixXd pos_pts = all_pos_trajs[i].getControlPoint();
  //   for (int j=0; j<pos_pts.rows(); ++j)
  //   {
  //     pos_pts_all(counter_p, 0) = pos_pts(j, 0);
  //     pos_pts_all(counter_p, 1) = pos_pts(j, 1);
  //     pos_pts_all(counter_p, 2) = pos_pts(j, 2);
  //     counter_p++;
  //   }
  //   Eigen::VectorXd knots = all_pos_trajs[i].getKnot();
  //   for (int k=0; k<knots.rows(); ++k)
  //   {
  //     knots_all(counter_k) = knots(k);
  //     counter_k++;
  //   }
  // }
  for (int i=0; i<ctrl_pts_all.size(); ++i)
  {
    for (int j=0; j<ctrl_pts_all[i].rows(); ++j)
    {
      pos_pts_all(counter_p, 0) = ctrl_pts_all[i](j, 0);
      pos_pts_all(counter_p, 1) = ctrl_pts_all[i](j, 1);
      pos_pts_all(counter_p, 2) = ctrl_pts_all[i](j, 2);
      counter_p++;
    }
  }
  double dt_all = duration_/seg_size; 

  NonUniformBspline final_pos;
  final_pos.setUniformBspline(pos_pts_all, pp_.bspline_degree_, dt_all);
  local_data_.position_traj_ = final_pos;
...
void FastPlannerManager::updateTrajInfo() {
  ...
  // local_data_.end_vel_ = local_data_.velocity_traj_.evaluateDeBoorT(local_data_.duration_);
  // local_data_.end_acc_ = local_data_.acceleration_traj_.evaluateDeBoorT(local_data_.duration_);
...
void FastPlannerManager::planYawExplore(const Eigen::Vector3d& start_yaw, const double& end_yaw,
    bool lookfwd, const double& relax_time, const vector<double>& timestep, const vector<double>& mid_yaws) {
  ...
  vector<int> waypt_idx;
  if (lookfwd) {
    const double forward_t = 0.5;
    const int relax_num = relax_time / dt_yaw;
    for (int i = 1; i < seg_num - relax_num; ++i) {
      double tc = i * dt_yaw;
      Eigen::Vector3d waypt;
      double yaw_cur = inter_yaw(timestep, mid_yaws, tc);
      // cout << "Yaw_cur:" << yaw_cur*180.0/M_PI << " deg." << endl;
      waypt(0) = yaw_cur;
      waypt(1) = waypt(2) = 0.0;
      calcNextYaw(last_yaw, waypt(0));
  ...
  local_data_.control_pts_yaw = yaw;
  // local_data_.yaw_traj_.setUniformBspline(yaw, 3, 0.1);
  local_data_.yawdot_traj_ = local_data_.yaw_traj_.getDerivative();
  local_data_.yawdotdot_traj_ = local_data_.yawdot_traj_.getDerivative();
  plan_data_.dt_yaw_ = dt_yaw;
  
  local_data_.end_yaw_(0) = local_data_.yaw_traj_.evaluateDeBoorT(local_data_.duration_)(0);
  local_data_.end_yaw_(1) = local_data_.yawdot_traj_.evaluateDeBoorT(local_data_.duration_)(0);
  local_data_.end_yaw_(2) = local_data_.yawdotdot_traj_.evaluateDeBoorT(local_data_.duration_)(0);
  // plan_data_.path_yaw_ = path;
  // plan_data_.dt_yaw_path_ = dt_yaw * subsp;
}

void FastPlannerManager::planYawFrontier(const Eigen::Vector3d& start_yaw, const double& end_yaw,
...
double FastPlannerManager::inter_yaw(const vector<double>& timeset, const vector<double>& yaws, const double& timestamp)
{

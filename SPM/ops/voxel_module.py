...
class Voxelization(nn.Module):
    ...
    def __init__(self,
        ...
        """
        Args:
            voxel_size (list_float): list [x, y, z] size of three dimension, the size of each basic voxel in each axis
            point_cloud_range (list_float):
...
if __name__ == '__main__':
    ...
    size_range = [0.0, 0.0, 0.0, x_range, y_range, z_range]
    max_num_pt = 3
    max_voxels = (3000, 3000)

    cloud = torch.randn(15000, 3)
    cloud[:,0] = x_range*cloud[:,0]
    cloud[:,1] = y_range*cloud[:,1]
    cloud[:,2] = y_range*cloud[:,2]

    cloud_array = cloud.numpy()

    voxel_layer = Voxelization(voxel_size=voxel_size, point_cloud_range=size_range, max_num_points=max_num_pt, max_voxels=max_voxels)

    voxel,coords,num = voxel_layer.forward(cloud)
    print(voxel.size())
    print(coords.size())
    print(num.size())
    voxel_array = voxel.numpy()
    # vis_input_pcd(cloud_array)
    vis_voxelization(voxel_array)
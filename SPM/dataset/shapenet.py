...
class ShapeNet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        partial_pc = partial_pc/(self.normal_size)
        complete_pc = complete_pc/(self.normal_size)
        # ----------------------------
        return torch.from_numpy(partial_pc).type(torch.FloatTensor), torch.from_numpy(complete_pc).type(torch.FloatTensor), center, self.scale
    ...
    def random_aug(self, pc, n):
        if pc.shape[0] < n:
            pc_out = pc
        else:
            pt_num = pc.shape[0]
            mask_prob = 0.2+0.2*random.random()
            mask_num = int(mask_prob*pt_num)
            start_id = random.randint(0,pt_num-1)
            front = pc[:start_id]
            back = pc[start_id+mask_num:]
            pc_out = np.concatenate([front, back], axis=0)
